...
export const getBinaryNodeFilter = (additionalNodes, content) => {
  const node = [...additionalNodes]
  const indexObject = node.findIndex(item => JSON.stringify(item) === JSON.stringify(content));

  if (indexObject !== -1) {
    node.splice(indexObject, 1);
    node.push(content); 
    return node;
  } else {
    node.push(content);
